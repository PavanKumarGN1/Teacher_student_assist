...
def generate_summary_for_chunk(chunk_content):
    ...
    if response.status_code == 200:
        ...
        st.error(f"Failed to generate summary: {response.status_code} - {response.text}")
        return None
...
def generate_questions_and_answers(summary):
    data = {
        ...
        "messages": [
            ...
            {
                "role": "user",
                "content": f"Generate 5 questions and their answers from the following summary: {summary}"
            }
        ]
    }
...
def save_qna_to_excel(qna):
    df = pd.DataFrame({"Questions and Answers": [qna]})
    output = BytesIO()
...
def main():
    ...
    if uploaded_file is not None:
        ...
        if document_content:
            ...
            # "Done" button to generate questions and answers
            if st.button("Done"):
                st.success("Generating questions and answers...")
                questions_answers = generate_questions_and_answers(st.session_state.summary)
                ...
                if questions_answers:
                    ...
                    # Save Q&A to Excel
                    excel_data = save_qna_to_excel(questions_answers)
                    st.download_button(
                        label="Download Q&A as Excel",
                        data=excel_data,
                        file_name="questions_and_answers.xlsx",
